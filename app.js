...
// ========= Traducción automática EN->FR si falta el campo FR =========
...
function cacheKey(text, from, to){
  let h = 0;
  for (const c of `${from}:${to}:${text}`) h = (h * 31 + c.charCodeAt(0)) >>> 0;
  return String(h);
}
...
// ========= Utilidades de imágenes seguras + fallback =========
...
// ========= Render del artículo principal (lead) =========
async function renderLead(a){
  ...
  // Título + bajada con traducción si hace falta
  ...
  const meta = el('div','excerpt'); meta.textContent = `${a.category} • ${formatDate(a.date)}`;
  box.appendChild(h); box.appendChild(d); box.appendChild(meta);

  // Cuerpo + galería en dos columnas
...
async function renderLatestItem(a){
  ...
  const meta = el('div','muted'); meta.textContent = `${a.category} • ${formatDate(a.date)}`;
...
async function render(){
  ...
  if (!vids.length){ videoList.textContent = '—'; }
    ...
    for (const v of vids.slice(0,3)){
      ...
      let text = (viewMode==='fr' ? (v.title.fr || await translateText(v.title.en||'', 'en', 'fr')) : v.title.en || v.title.fr);
      item.textContent = text || '';
      videoList.appendChild(item);
    }
  }

  // Textos fijos (evitamos optional chaining a la izquierda)
  const elAbout = document.querySelector('[data-i18n="about_title"]');
  if (elAbout) {
    elAbout.textContent = (viewMode==='fr'
  ...
  if (elBody) {
    elBody.textContent = (viewMode==='fr'
  ...
  if (elLatest) {
    elLatest.textContent = (viewMode==='fr'
...
async function loadContent(){
  try{
    ...
    console.log(`Loaded ${ARTICLES.length} article(s) from ${CONTENT_URL}`);
  }catch(e){
